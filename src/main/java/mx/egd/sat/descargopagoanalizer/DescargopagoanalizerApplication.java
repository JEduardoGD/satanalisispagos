...
public class DescargopagoanalizerApplication implements CommandLineRunner {
	...
	@Autowired
	private LogsAnalizerService logsAnalizerService;
	@Autowired
	private CifrasService cifrasService;
	@Autowired
	private ReportePorFechaService reportePorFechaService;
	@Autowired
	private AnalizaCifrasControlService analizaCifrasControlService;
	@Autowired
	private GeneraInformeService generaInformeService;
	...
	public void run(String... args) throws Exception {
		if (args != null && args.length == 1) {
			...
			List<ObjAnalisisPagadosFecha> objAnalisisPagadosFechaList = reportePorFechaService
					.createReporteFechas(registrosLog, listCreditosFiscales);
			...
			if (isFile != null && isFile.booleanValue()) {
				analizaCifrasControlService.creaListaCifrasControlFile(args[0]);
			}
			if (isFolder != null && isFolder.booleanValue()) {
				List<Registro> finalLista = null;
				if (args[2] == null) {
					log.error("Falta el parametro 3: [porcarpeta|porarchivo]");
					return;
				}
				...
				case POR_ARCHIVO:
					...
					for (File file : filesOnFolderList) {
						...
						if (cifracontrolList != null && !cifracontrolList.isEmpty()) {
